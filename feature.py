...
class FeatureAgent(MahjongGBAgent):
    ...
    OFFSET_OBS = {
        ...
        'PREVALENT_WIND' : 1,   # 场风
        'HAND' : 2, # 手牌
        'rival_ChiPengGang' : 6,    # 玩家吃碰杠牌
        'AnGang' : 30, # 暗杠
        'rival_QiPai' : 31, # 玩家弃牌情况
        'YAMA' : 143 # 剩余牌
    }
    # ACT 的偏移，也是写死的，不会变动
    OFFSET_ACT = {
    ...
    def __init__(self, seatWind):
        ...
        # 初始化所有剩余牌为1
        array1 = np.ones((4,36))
        self.obs[self.OFFSET_OBS['YAMA']:] = array1
        # 初始化牌山
        self.yama = np.ones((4,9)) * 4
        self.yama[3][7] = 0
        self.yama[3][8] = 0
    ...
    def request2obs(self, request):
        ...
        if t[0] == 'Deal':
            ...
            for card in self.hand:
                self._remove_card_from_yama(card)
            self._yama_embedding_update()
        ...
        if t[0] == 'Draw':
            ...
            self._remove_card_from_yama(tile)
            self._hand_embedding_update()
            self._yama_embedding_update()
        ...
        if t[2] == 'Play':
            ...
            # 计算弃牌历史的通道索引并更新
            channel_idx = self.OFFSET_OBS['rival_QiPai'] + (p - 1) * 28 + len(self.history[p]) - 1
            self.obs[channel_idx][self.OFFSET_TILE[self.curTile]] = 1

            self._remove_card_from_yama(self.curTile)
            self._yama_embedding_update()
        ...
        if t[2] == 'Chi':
            ...
            self.wallLast = self.tileWall[(p + 1) % 4] == 0
            # 更新第p个玩家吃牌的通道索引
            channel_idx = self.OFFSET_OBS['rival_ChiPengGang'] + (p - 1) * 6 + self.ChiNum[p] - 1
            self.obs[channel_idx][self.OFFSET_TILE[self.curTile]] = 1
        ...
        if t[2] == 'UnChi':
            ...
            # 更新第p个玩家吃的吃牌的通道索引
            channel_idx = self.OFFSET_OBS['rival_ChiPengGang'] + (p - 1) * 6 + self.ChiNum[p] - 1
            self.obs[channel_idx][self.OFFSET_TILE[self.curTile]] = 0
            # 如果是自己取消吃牌
            if p == 0:
                for i in range(-1, 2):
        ...
        if t[2] == 'Peng':
            ...
            self.wallLast = self.tileWall[(p + 1) % 4] == 0
            # 更新第p个玩家吃的碰牌的通道索引
            channel_idx = self.OFFSET_OBS['rival_ChiPengGang'] + (p - 1) * 6 + 4
            self.obs[channel_idx][self.OFFSET_TILE[self.curTile]] = 1
            if p == 0:
                # 可选动作：Play
                self.valid = []
        ...
        if t[2] == 'Gang':
            ...
            # 更新杠牌的通道索引
            channel_idx = self.OFFSET_OBS['rival_ChiPengGang'] + (p - 1) * 6 + 5
            self.obs[channel_idx][self.OFFSET_TILE[self.curTile]] = 1
            if p == 0:
                for i in range(3):
                    self.hand.remove(self.curTile)
        ...
        if t[2] == 'AnGang':
            ...
            # 更新己方暗杠的通道索引
            if p == 0:
                channel_idx = self.OFFSET_OBS['AnGang']
        ...
        if t[2] == 'BuGang':
            ...
            # 更新杠和碰的通道索引
            channel_idx_gang = self.OFFSET_OBS['rival_ChiPengGang'] + (p - 1) * 6 + 5
            channel_idx_peng = self.OFFSET_OBS['rival_ChiPengGang'] + (p - 1) * 6 + 4
            self.obs[channel_idx_gang][self.OFFSET_TILE[tile]] = 1
            self.obs[channel_idx_peng][self.OFFSET_TILE[tile]] = 0
            if p == 0:
                self.hand.remove(tile)
                self._hand_embedding_update()
                self.isAboutKong = True
    ...
    def _remove_card_from_yama(self, card):
        """
        从牌山移除一张牌。
        """
        idx = self.OFFSET_TILE[card]
        row, col = divmod(idx, 9)
        self.yama[row, col] -= 1

    def _yama_embedding_update(self):
        """
        根据 self.yama 更新 self.obs 矩阵中的牌山信息部分，以反映牌山剩余的牌。(实现的有问题)
        前接 deal
        """
        # 清空牌山信息
        self.obs[self.OFFSET_OBS['YAMA'] : ] = 1
        d = np.zeros((4,9))
        for row in range(4):
            for col in range(9):
                d[row, col] = 4 - self.yama[row, col]
                self.obs[self.OFFSET_OBS['YAMA'] : self.OFFSET_OBS['YAMA'] + int(d[row, col]), row * 9 + col] = 0
